...
import studio.creater120.hapai.data.UISettings
import studio.creater120.hapai.utility.DomainListUtils
import studio.creater120.hapai.utility.checkIpAndPortInCmd
import studio.creater120.hapai.utility.getCmdEnable
import studio.creater120.hapai.utility.getStringNotNull
import studio.creater120.hapai.utility.shellSplit

sealed interface HapaiProxyPreferences {
    companion object {
        fun fromSharedPreferences(preferences: SharedPreferences, context: Context): HapaiProxyPreferences =
            when (preferences.getCmdEnable()) {
                true -> HapaiProxyCmdPreferences(preferences, context)
                false -> HapaiProxyUIPreferences(preferences)
            }
    }
}

class HapaiProxyCmdPreferences(val args: Array<String>) : HapaiProxyPreferences {
...
class HapaiProxyUIPreferences(val settings: UISettings = UISettings()) : HapaiProxyPreferences {
