...
import studio.creater120.hapai.activities.MainActivity
import studio.creater120.hapai.core.HapaiProxy
import studio.creater120.hapai.core.HapaiProxyPreferences
...
class HapaiVpnService : LifecycleVpnService() {
    private val HapaiProxy = HapaiProxy()
    private var proxyJob: Job? = null
    private var tunFd: ParcelFileDescriptor? = null
    private var vlessConfigJson: String? = null
    private val mutex = Mutex()

    companion object {
        private val TAG: String = HapaiVpnService::class.java.simpleName
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        vlessConfigJson = intent?.getStringExtra(EXTRA_CONFIG_JSON)
        ...
        return when (val action = intent?.action) {
            ...
            RESUME_ACTION -> {
                lifecycleScope.launch {
                    if (prepare(this@HapaiVpnService) == null) {
                        start()
                    }
                }
                START_STICKY
    ...
    private fun startProxy() {
        ...
        val configJson = vlessConfigJson
        if (configJson != null) {
            Log.i(TAG, "Starting VLESS proxy via sing-box")

            proxyJob = lifecycleScope.launch(Dispatchers.IO) {
                try {
                    delay(500)
                    val result = happicore.Happicore.startProxy(configJson)
                    Log.i(TAG, "VLESS proxy result: $result")
                    if (result != "ok") {
                        Log.e(TAG, "VLESS proxy error: $result")
                    ...
                    Log.e(TAG, "VLESS proxy exception", e)
                    withContext(Dispatchers.Main) {
                        updateStatus(ServiceStatus.Failed)
                        stopTun2Socks()
                        stopSelf()
            ...
            proxyJob = lifecycleScope.launch(Dispatchers.IO) {
                val code = HapaiProxy.startProxy(preferences)
    ...
    private suspend fun stopProxy() {
        ...
        try {
            if (vlessConfigJson != null) {
                Log.i(TAG, "Stopping VLESS proxy")
                happicore.Happicore.stopProxy()
            } else {
                HapaiProxy.stopProxy()
            }
            proxyJob?.cancel()
            ...
            if (completed == null) {
                ...
                if (vlessConfigJson != null) {
                    // Happicore.stopProxy already handles cleanup
                } else {
                    HapaiProxy.jniForceClose()
                }
            }
    ...
    private fun getByeDpiPreferences(): HapaiProxyPreferences =
        HapaiProxyPreferences.fromSharedPreferences(getPreferences(), this)
